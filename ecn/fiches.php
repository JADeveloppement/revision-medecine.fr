	...
	if (!isset($_COOKIE['ECN-SUIVI-nom']) || !isset($_COOKIE['ECN-SUIVI-var']))
		...
		for ($a = 0; $a < 11; $a++){
			for ($b = 0; $b < $nbrelupermodule[$a][1]; $b++){
				...
				array_push($tabrelu, intval(trim($d[0])));
				...
				<div class="card shadow-sm position-relative mt-4 mb-4 mr-5" style="width: 20rem;">
					...
					<div class="card-body bg-light text-muted">
						...
						<div class="progress mt-3">
							<div id="pUE1" class="progress-bar text-center text-dark bg-secondary" role="progressbar" style="width: <? echo (($nbrelupermodule[$a][0]/$nbrelupermodule[$a][1])*100); ?>%" aria-valuemin="0" aria-valuemax="100"><? echo ($nbrelupermodule[$a][0])." items fichés sur ".$nbrelupermodule[$a][1]; ?></div>
						</div>
					</div>
						...
						<ul class="list-group">
							<?
								if ($nbrelupermodule[$a][0] == 0){
									echo "<li class='list-group-item list-group-item-danger'>Aucun item fiché dans ce module.</li>";
								}
								for ($b = 0; $b < $nbrelupermodule[$a][1]; $b++){
									if ($tabrelu[$c] >= 1){
							?>
								<li class="list-group-item list-group-item-success"><?echo $listeitem[$c]; ?></li>
							<?
									}
									$c++;
								}
				...
				<div class="card shadow-sm position-relative mt-4 mb-4 mr-5" style="width: 20rem;">
					...
					<div class="card-body bg-light">
						...
						<div class="progress mt-3">
							...
							<div id="pUE1" class="progress-bar text-center text-dark <? echo $color; ?>" role="progressbar" style="width: <? echo (($nbrelupermodule[$a][0]/$nbrelupermodule[$a][1])*100); ?>%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"><? echo ($nbrelupermodule[$a][0])." items fichés sur ".$nbrelupermodule[$a][1]; ?></div>
						</div>
					</div>
		...
		<script>
			$("input[name='search']").keyup(function (e) {
				if ($(this).val().length == 0)
					$("#chercheue").collapse('hide');
				if ($(this).val().length >= 1){
					$("#chercheue").collapse('show');
					$("li[abs='search']").each(function(){
						if ($(this).html().toLowerCase().search($("input[name='search']").val().toLowerCase()) !== -1)
							$(this).removeClass('sr-only');
						else {
							$(this).removeClass('sr-only').addClass('sr-only');
						}
					})
				}
				else {
